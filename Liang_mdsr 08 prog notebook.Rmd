...
knn_error_rate <-
  function(x, y, numNeighbors, z = x) {
    ...
    ### x: train
    ### y: income
    ### numNeighbors: what to set k to
    ### z: what to set test to
    # output:
    ### k value
    y_hat <-
      knn(train = x, test = z, cl = y, k = numNeighbors)
...
get_roc <-
  function(x, y) {
    ...
    # inputs:
    ### x: data frame with y_hat values for various models
    ### y: income
    # output:
    ### data for ROC curves
...
# p. 176
form <- 
  as.formula('income ~ age + workclass + education + marital.status +
...
train <-
  train %>%
  mutate(husband_or_wife = relationship %in%
           c(' Husband', ' Wife'),
         college_degree = husband_or_wife & education %in%
           c(' Bacehlors', ' Doctorate', ' Masters', ' Prof-school'),
         income_dtree = predict(mod_tree, type = 'class'))

cg_splits <-
  data.frame(husband_or_wife = c(TRUE, FALSE),
...
confusiondtree <-
  tally(income_dtree ~ income,
        data = train, 
        format = 'count')
confusiondtree
sum(diag(confusiondtree)) / nrow(train)
```
...
# p. 181
mod_tree2 <-
  rpart(form, data = train, control = rpart.control(cp = 0.002))
```
...
mod_forest <-
  form %>%
  randomForest(data = train, ntree = 201, mtry = 3)
mod_forest
...
income_knn <-
  train_q %>%
  knn(test = train_q, cl = train$income, k = 10)

confusionknn <- 
  tally(income_knn ~ income, data = train, format = 'count')
confusionknn

sum(diag(confusionknn)) / nrow(train)
```
...
ks <-
  c(1:15, 20, 30, 40, 50)
train_rates <-
  sapply(ks, FUN = knn_error_rate, x = train_q, y = train$income)
knn_error_rates <-
  data.frame(k = ks, train_rate = train_rates)
ggplot(data = knn_error_rates,
       aes(x = k, y = train_rate)) +
  geom_point() +
  geom_line() +
...
confusionnb <-
  tally(income_nb ~ income,
        data = train,
        format = 'count')
confusionnb
sum(diag(confusionnb)) / nrow(train)
```
...
sum(diag(confusionens)) / nrow(train)
```
...
# p. 190
pred <-
  ROCR::prediction(income_probs[,2], train$income)
perf <-
...
# p. 191
perf_df <-
  data.frame(perf@x.values, perf@y.values)
names(perf_df) <-
  c('fpr', 'tpr')
roc <-
  ggplot(data = perf_df,
...
test_q <-
  ...
  select(age, education.num, capital.gain, capital.loss, hours.per.week)
test_rates <-
  sapply(ks, FUN = knn_error_rate, x = train_q, y = train$income, z = test_q)
knn_error_rates <- 
  knn_error_rates %>%
...
ggplot(data = knn_error_rates_tidy,
       aes(x = k, y = error_rate)) +
...
# p. 193
mod_null <-
  glm(income ~ 1, data = train, family = binomial)
mods <-
  list(mod_null, mod_tree, mod_forest, mod_nn, mod_nb)
```
...
predictions <-
  bind_rows(predictions_train, predictions_test)
glimpse(predictions)
```
...
```{r}
# p. 196
ggplot(data = roc_tidy,
       aes(x = fpr, y = tpr)) +
  geom_line(aes(color = model)) +
  geom_abline(intercept = 0, slope = 1, lty = 3) +
  geom_point(data = predictions_summary, 
             size = 3,
...
# p. 197
whoIsDiabetic <-
  rpart(Diabetes ~ Age + BMI + Gender + PhysActive,
        data = people,
...
ggplot(data = people,
  ...
  annotate('rect', xmin = 60.5, xmax = 67.5, ymin = 39.985, ymax = Inf,
           fill = 'blue', alpha = 0.1)
```
...
# p. 199
ages <-
  range(~ Age, data = people)
bmis <-
  range(~ BMI, data = people)
res <- 
  100
fake_grid <-
  expand.grid(
...
dmod_forest <-
  formBMI %>%
  randomForest(data = people, ntree = 201, mtry = 2)
dmod_nnet <-
  formBMI %>%
  nnet(data = people, size = 6)
dmod_nb <-
  formBMI %>%
  naiveBayes(data = people)

pred_tree <-
  predict(dmod_tree, newdata = fake_grid)[, 'Yes']
pred_forest <-
  predict(dmod_forest, newdata = fake_grid, type = 'prob')[, 'Yes']
pred_knn <-
  people %>%
  select(Age, BMI) %>%
  knn(test = select(fake_grid, Age, BMI),
      cl = people$Diabetes,
      k = 5) %>%
  as.numeric() - 1
pred_nnet <-
  ...
  as.numeric()
pred_nb <-
  predict(dmod_nb, newdata = fake_grid, type = 'raw')[, 'Yes']
```

```{r}
# p. 200
p <-
  tally(~ Diabetes, data = people, format = 'proportion')['Yes']
p
```
